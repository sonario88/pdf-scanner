# field_processors/TM_corob_processor.py
"""
Модуль для предобработки и OCR изображений поля 'TM_corob' с главной этикетки партии (page1_mane.png).
Сохраняет обработанное изображение в папку fields_edited/mane/TM_corob.
"""
import os
import logging
...
def preprocess_and_ocr_TM_corob_field(
    ...
    """
    Предобрабатывает изображение поля 'TM_corob' и выполняет OCR.
    Всегда удаляет префикс "ТМ:" из распознанного текста.

    Args:
        pil_image (PIL.Image): Исходное изображение поля.
        original_filename (str, optional): Имя оригинального файла (например, page1_mane_field_TM_corob.png).
        page_num (int, optional): Номер страницы.
        label_num (int, optional): Номер этикетки (0 для mane).

    Returns:
        tuple: (обработанное_pil_image, распознанный_текст_без_"ТМ:")
               Возвращает только очищенный текст.
    """
    try:
        logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Начало УПРОЩЕННОЙ обработки (увеличение 3x)...")
        ...
        logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Преобразовано в OpenCV Mat.")
        ...
        logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Изображение увеличено до {new_dimensions} (3x).")

        # 3. Медианная фильтрация для уменьшения шума (опционально)
        # denoised = cv2.medianBlur(resized, 3)
        # logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Применена медианная фильтрация.")
        ...
        logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Изображение преобразовано обратно в PIL.")
        ...
        logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] OCR завершен. Сырой текст: '{raw_text}'")
        
        # --- Постобработка текста: УДАЛЕНИЕ ПРЕФИКСА ---
        logger.debug("Начало постобработки текста для поля 'TM_corob'")
        
        # 1. Базовая очистка
        cleaned_text_stage1 = raw_text.strip()
        
        # 2. Удаление префикса "ТМ:" с помощью регулярного выражения
        # ^[Тт][Мм]\s*:?\s* - ТМ (с учетом регистра) и возможное двоеточие
        pattern = r'^[Тт][Мм]\s*:?\s*'
        ...
        for prefix in prefixes_to_remove:
            if cleaned_text_stage2.lower().startswith(prefix.lower()):
                ...
                logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Удален АЛЬТЕРНАТИВНЫЙ префикс '{prefix}'. Текст после очистки: '{final_text}'")
                break # Прекращаем перебор после первого совпадения
        
        if final_text == cleaned_text_stage2:
            logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Префикс 'ТМ:' не найден (ни регулярным выражением, ни вручную). Текст остался без изменений.")
        else:
            logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Префикс 'ТМ:' успешно удален. Финальный текст: '{final_text}'")
            
        logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Финальный очищенный текст: '{final_text}'")
        # --- КОНЕЦ Постобработки ---
        ...
        if FIELD_OUTPUT_DIR and original_filename:
            try:
                ...
                logger.debug(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Попытка сохранить обработанное изображение как: {save_path}")
                ...
                logger.info(f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Обработанное изображение поля 'TM_corob' сохранено: {save_path}")
            except Exception as save_error:
                error_msg = f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Ошибка при сохранении обработанного изображения поля 'TM_corob': {save_error}"
        ...
        error_msg = f"[Поле 'TM_corob' на стр. {page_num}, этикетка 'mane'] Ошибка при УПРОЩЕННОЙ обработке или OCR изображения поля 'TM_corob': {e}"
        logger.error(error_msg, exc_info=True)
...
def preprocess_and_ocr_field(
    ...
    """
    return preprocess_and_ocr_TM_corob_field(pil_image, original_filename, page_num, label_num)
