# field_processors/country_corob_processor.py
"""
Модуль для предобработки и OCR изображений поля 'country_corob' с главной этикетки партии (page1_mane.png).
Сохраняет обработанное изображение в папку fields_edited/mane/country_corob.
"""
import os
import logging
...
def preprocess_and_ocr_country_corob_field(
    ...
    """
    Предобрабатывает изображение поля 'country_corob' и выполняет OCR.

    Args:
        pil_image (PIL.Image): Исходное изображение поля.
        original_filename (str, optional): Имя оригинального файла (например, page1_mane_field_country_corob.png).
    ...
    """
    try:
        logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Начало УПРОЩЕННОЙ обработки (увеличение 3x)...")
        ...
        logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Преобразовано в OpenCV Mat.")
        ...
        logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Изображение увеличено до {new_dimensions} (3x).")

        # 3. Медианная фильтрация для уменьшения шума (опционально)
        # denoised = cv2.medianBlur(resized, 3)
        # logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Применена медианная фильтрация.")
        ...
        logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Изображение преобразовано обратно в PIL.")
        ...
        logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] OCR завершен. Сырой текст: '{raw_text}'")
        
        final_text = raw_text.strip()
        logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Финальный очищенный текст: '{final_text}'")
        # --- КОНЕЦ OCR ---
        ...
        if FIELD_OUTPUT_DIR and original_filename:
            try:
                ...
                logger.debug(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Попытка сохранить обработанное изображение как: {save_path}")
                ...
                logger.info(f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Обработанное изображение поля 'country_corob' сохранено: {save_path}")
            except Exception as save_error:
                error_msg = f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Ошибка при сохранении обработанного изображения поля 'country_corob': {save_error}"
        ...
        error_msg = f"[Поле 'country_corob' на стр. {page_num}, этикетка 'mane'] Ошибка при УПРОЩЕННОЙ обработке или OCR изображения поля 'country_corob': {e}"
        logger.error(error_msg, exc_info=True)
...
def preprocess_and_ocr_field(
    ...
    """
    return preprocess_and_ocr_country_corob_field(pil_image, original_filename, page_num, label_num)
